        ...
        <div class="window-content public-window">
            <div class="window-body">
                        ...
                        <ul>
                            <li v-for="(item, index) in myModels.list" :key="index"
                                ...
                                <div class="model-body">
                                    <h4 :title="item.name">{{ dealString(item.name, 15)}}</h4>
                                    <div class="stars">
                                        <!--<span v-for="(star,index) in item.modelLevel" :class="{'check':star==='1'}"-->
                                              <!--:key="index"-->
                                              <!--@click="setRate(star,item,index)">-->
                                        <!--</span>-->
                                        <span v-for="(star,index) in item.modelLevel" :class="{'check':star==='1'}"
                                              :key="index">
                                        </span>
                                    ...
                                    <div class="upload-doc">
                                        ...
                                        <input type="file" ref="uploadFile" @change="toUpload(item,index)"
                ...
                <div class="model-select-zone ">
                    ...
                    <!-- 选择模型价值点区域 -->
                    <div class="values-zone  window-body-section">
                        <div class="tags-header section-header">
                            <h5>3.选择模型价值点</h5>
                        </div>
                    ...
                    <div class="police-class-list">
                        <ul>
                            <li v-for="(item, index) in policeList.list"
                                ...
                                v-bind:key="index">
                                <span class="police-radio"
                                      :class="{'active': policeList.actPolice.ID === item.ID}"></span>
            ...
            <div class="publish-btn-area">
                <div class="public-btn-div" @click="toPublish()">
                    确认
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        getModels () {
            ...
            const array = typeof this.item.modelLevel === 'string' ? JSON.parse(this.item.modelLevel) : this.item.modelLevel
            this.item.modelLevel = array
            this.myModels.list.push(this.item)
        },
        getAllPoint () {
        ...
        // 设置价值点
        setvps (array) {
            const vps = this.item.vps
            const len = array.length
            const len2 = vps.length
            for (let i = 0; i < len; i++) {
                for (let j = 0; j < len2; j++) {
                    if (vps[j] === array[i].NAME) {
                        array[i].check = true
                        this.valuesList.checkList.push(array[i].ID)
                    }
                }
            }
            this.valuesList.list = array
        },
        // 设置选中的标签
        setTags (array) {
            const tags = this.item.tags
            this.tagsList.checkAll = tags.length === array.length
            const len = array.length
            const len2 = tags.length
            for (let i = 0; i < len; i++) {
                for (let j = 0; j < len2; j++) {
                    if (array[i].NAME === tags[j]) {
                        array[i].checked = true
                    }
                }
            }
            this.tagsList.list = array
        },
        getAllTabs () {
            serve.knowledge.getTabs({ name: this.tagsList.searchTags }).then(res => {
                const array = res.map((item, index, arr) => {
                    item.checked = false
                    return item
                })
                this.setTags(array)
        ...
        // 设置选中的警种
        setJz (array) {
            const jzId = this.item.jzId ? this.item.jzId.toString() : ''
            if (!jzId) {
                return
            }
            const len = array.length
            for (let i = 0; i < len; i++) {
                if (jzId === array[i].ID) {
                    this.policeList.actPolice = array[i]
                }
            }
        },
        // 关闭窗口
        ...
        // 星级
        setRate (star, model, index) {
            const starAry = model.modelLevel.slice(0, index + 1)
            const contAry = model.modelLevel.slice(index + 1, model.modelLevel.length)
            if (star === '0') {
                for (let i = 0; i < starAry.length; i++) {
                    starAry[i] = '1'
                }
            } else {
                for (let i = 0; i < contAry.length; i++) {
                    contAry[i] = '0'
                }
            }
            model.modelLevel = starAry.concat(contAry)
        },
        // 上传文档
        toUpload (model, i) {
        ...
        toPublish () {
            ...
            console.log(params)
            params.isPublic = 1
            params.vps = this.valuesList.checkList.map((item) => {
            ...
            serve.knowledge.publish(params).then(res => {
                this.closeWindow()
                util.showMessage('模型修改成功！', 'success')
                this.$emit('publish')
            }).catch(err => {
    ...
    created () {
        this.myModels.activeModel = this.item
    },
    mounted () {
        this.getModels()
        this.getAllPoint()
